...
class VSSClient(BaseVSSClient):
    ...
    async def authorize(self, token: str, **rpc_kwargs) -> str:
        ...
        server_info = await self.get_server_info(**rpc_kwargs)
    ...
    async def get_server_info(self, **rpc_kwargs) -> ServerInfo:
        ...
        try:
            ...
            if exc._code != grpc.StatusCode.UNAUTHENTICATED:
                raise VSSClientError.from_grpc_error(exc) from exc
            else:
                logger.info(
                    "Unauthenticated channel started. To specify token use authorize <FILE> client commands")
                resp = type('Unauthenticated', (), {})
                resp.name = "Unauthenticated"
                resp.version = 1
        logger.debug("%s: %s", type(resp).__name__, resp)
